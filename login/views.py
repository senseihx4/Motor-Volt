...
def listings(request):
    ...
    cars_list = cars.objects.filter(is_approved=True).order_by('-created_at')

    if fuel_type:
        cars_list = cars_list.filter(fuel_type__iexact=fuel_type)  

    return render(request, 'listings.html', {'cars': cars_list})
